...
export function hibRecommendation(normalizedName, birthDate, currentDate, validDoses, numDoses, sortedDoses, specialConditions, immunityEvidence) {
    ...
    if (currentAgeYears >= 5) {
        ...
        if (ageAtFirst < 7)
            ageKey = '<7m';
        else if (ageAtFirst < 12)
            ageKey = '7-11m';
        else if (ageAtFirst < 15)
            ageKey = '12-14m';
        else if (ageAtFirst < 60)
            ageKey = '15-59m';
        else
            ageKey = '15-59m';

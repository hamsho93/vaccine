...
export function getAgeInDays(birthDate: Date, currentDate: Date): number {
...
export function getAgeInYears(birthDate: Date, currentDate: Date): number {
  const ageInDays = getAgeInDays(birthDate, currentDate);
  return Math.floor(ageInDays / 365.25);
}

export function isDoseTooEarly(vaccineName: string, doseNumber: number, birthDate: Date, doseDate: Date, previousDoseDate: Date | null): boolean {
...
export class VaccineCatchUpService {
  ...
  ): 'normal-schedule' | 'catch-up-schedule' | 'unknown' {
    if (validDoses.length === 0) return 'unknown';
    
    // Define normal schedule windows for common vaccines
    const normalSchedules: Record<string, { dose: number, minAgeMonths: number, maxAgeMonths: number }[]> = {
      'hepatitis_b': [
  ...
  private getVaccineRecommendation(
    ...
    let vaccineRecommendation: VaccineRecommendation | null;
    ...
    for (const vaccine of standardVaccines) {
      ...
      if (!processedVaccines.has(normalizedVaccine)) {
        ...
        const isDuplicate = recommendations.some(r => 
          r && this.normalizeVaccineName(r.vaccineName) === normalizedVaccine
        );

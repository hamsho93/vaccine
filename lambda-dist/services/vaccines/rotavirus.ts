import { VaccineDoseInfo, getAgeInDays, getAgeInMonths, addDays, formatDate } from '../vaccine-catchup.ts';
import { VaccineRecommendation } from '../../../shared/schema.ts';
import { SpecialConditions } from '../vaccine-cdc-rules';

export function rotavirusRecommendation(
  ...
  const hasRotaTeq = validDoses.some(dose => 
    dose.product?.toLowerCase().includes('rotateq') || 
